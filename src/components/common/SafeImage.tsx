...
export const SafeImage: React.FC<SafeImageProps> = ({
  ...
  if (!hasError && src) {
    return (
      <div className={`relative overflow-hidden ${className}`}>
        ...
        />
        {isLoading && (
          <div className="absolute inset-0 flex items-center justify-center bg-slate-100/50 dark:bg-zinc-900/50 backdrop-blur-md animate-pulse">
  ...
  if (fallbackType === 'profile') {
    return (
      <div className={`relative overflow-hidden flex flex-col items-center justify-center ${className} ${
        isDark 
          ? 'bg-gradient-to-br from-zinc-900 via-neutral-950 to-[#0c0517] text-white border border-[#D4AF37]/30 shadow-[0_0_40px_rgba(109,40,217,0.25)]' 
        ...
        <div className="relative z-10 flex flex-col items-center justify-center text-center p-6">
          <div className={`relative w-28 h-28 rounded-full flex items-center justify-center mb-4 transition-transform duration-500 hover:scale-105 ${
            isDark 
              ? 'bg-gradient-to-tr from-[#6D28D9] to-[#D4AF37] p-[2px] shadow-lg shadow-[#6D28D9]/40' 
              : 'bg-gradient-to-tr from-[#00C896] to-[#7FFFD4] p-[2px] shadow-lg shadow-[#00C896]/30'
          }`}>
            <div className="w-full h-full rounded-full bg-white dark:bg-[#0d0d0d] flex items-center justify-center overflow-hidden">
            ...
            <div className="absolute -bottom-1 -right-1 w-7 h-7 rounded-full bg-emerald-500 dark:bg-amber-400 border-2 border-white dark:border-black flex items-center justify-center shadow-md">
          ...
          <span className="text-[11px] text-slate-500 dark:text-zinc-400 mt-2 px-3 py-1 rounded-full bg-slate-100 dark:bg-zinc-800/80 border border-slate-200 dark:border-zinc-700">
  ...
  if (fallbackType === 'project') {
    return (
      <div className={`relative overflow-hidden flex flex-col items-center justify-between p-6 ${className} ${
        isDark 
          ? 'bg-gradient-to-br from-[#121212] via-[#1a1128] to-[#0d0d0d] border border-[#D4AF37]/20 text-slate-100 shadow-xl' 
          : 'bg-gradient-to-br from-white via-slate-50 to-emerald-50/40 border border-[#00C896]/20 text-slate-800 shadow-lg'
      }`}>
        <div className="absolute -right-10 -top-10 w-36 h-36 rounded-full blur-2xl opacity-30 bg-[#00C896] dark:bg-[#6D28D9]" />
        ...
        <div className="my-auto z-10 flex flex-col items-center text-center py-4">
          <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-3 shadow-md ${
            isDark 
              ? 'bg-[#1e172f] border border-[#D4AF37]/30 text-[#D4AF37]' 
          ...
          <p className="text-[11px] text-slate-500 dark:text-zinc-400 mt-1">High Performance System</p>
        </div>

        {/* Bottom indicator */}
        <div className="w-full z-10 flex items-center justify-between text-[11px] text-slate-400 dark:text-zinc-500 font-mono">
          <span>&lt;Architect /&gt;</span>
  ...
  if (fallbackType === 'certificate') {
    return (
      <div className={`relative overflow-hidden flex flex-col items-center justify-between p-6 ${className} ${
        isDark 
          ? 'bg-gradient-to-br from-[#141414] via-[#1f162c] to-[#0a0a0a] border border-[#D4AF37]/25 text-white' 
        ...
        <div className="text-center my-3">
          <div className="text-xs uppercase tracking-widest font-mono text-slate-400 dark:text-zinc-400 mb-1">
        ...
        <div className="w-full pt-3 border-t border-slate-200/50 dark:border-zinc-800 flex items-center justify-between text-[10px] font-mono text-slate-500 dark:text-zinc-400">
  ...
  if (fallbackType === 'logo') {
    return (
      <div className={`relative overflow-hidden flex items-center justify-center ${className} ${
        isDark ? 'bg-[#121212] text-[#D4AF37]' : 'bg-emerald-50 text-[#00A57A]'
      }`}>
