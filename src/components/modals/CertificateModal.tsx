      ...
      >
        ...
        <motion.div
          ...
          className={`fixed inset-0 backdrop-blur-md cursor-pointer ${
            isDark ? 'bg-black/85' : 'bg-black/40'
          }`}
        />
        ...
        <motion.div
          ...
          className={`relative w-full max-w-5xl max-h-[94vh] rounded-3xl backdrop-blur-3xl border shadow-2xl z-10 overflow-hidden flex flex-col ${
            isDark
              ...
              : 'bg-white/95 border-[rgba(0,200,150,0.3)] text-slate-800 shadow-[0_20px_60px_rgba(0,0,0,0.2)]'
          }`}
        >
          {/* Top Control Bar */}
          <div className={`flex items-center justify-between px-3.5 sm:px-6 py-3 border-b shrink-0 gap-2 ${
            ...
            <div className="flex items-center gap-3 min-w-0 pr-2">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <h3 className={`text-xs sm:text-base font-extrabold truncate ${
                    isDark ? 'text-[#F8FAFC]' : 'text-slate-900'
                  }`}>
                    {certificate.title}
                  </h3>
                  <span className="hidden md:inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-extrabold uppercase font-mono bg-emerald-500/15 text-emerald-500 border border-emerald-500/30">
                    <ShieldCheck className="w-3 h-3" />
                    <span>Verified</span>
                  </span>
                </div>
                <div className={`flex items-center gap-2 text-[11px] truncate font-medium ${
                  isDark ? 'text-[#CBD5E1]' : 'text-slate-500'
                }`}>
                ...
                <button
                  ...
                  className={`p-1.5 sm:p-2 rounded-xl border transition-all cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed ${
                    isDark
                      ...
                      : 'bg-slate-100 text-slate-700 border-slate-200 hover:border-[#00C896]'
                  }`}
                >
              ...
              <button
                ...
                className={`hidden xs:inline-block px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-xl border text-[11px] sm:text-xs font-mono font-bold transition-all cursor-pointer ${
                  isDark
                    ...
                    : 'bg-emerald-50 text-[#00A57A] border-emerald-200 hover:border-[#00C896]'
                }`}
              >
          ...
          <div 
            className={`relative flex-1 overflow-auto p-3 sm:p-6 md:p-8 flex items-center justify-center min-h-[320px] max-h-[calc(94vh-130px)] select-none ${
              isDark ? 'bg-black/30' : 'bg-slate-100/60'
            }`}
          ...
          <div className={`px-4 sm:px-6 py-2.5 border-t flex flex-wrap items-center justify-between gap-2 text-xs font-medium shrink-0 ${
            isDark ? 'bg-[#0E0E14]/90 border-[rgba(212,175,55,0.2)]' : 'bg-white border-slate-200'
          }`}>
            <div className="flex items-center gap-3">
              ...
              {certificate.issueDate && (
                <span className={`hidden sm:inline ${isDark ? 'text-[#CBD5E1]' : 'text-slate-500'}`}>
                  Issued: {certificate.issueDate}
                </span>
              )}
              {certificate.credentialId && (
                <span className={`font-mono hidden md:inline ${isDark ? 'text-[#CBD5E1]' : 'text-slate-500'}`}>
              ...
              <a
                ...
                className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-xl text-[11px] font-bold border transition-all ${
                  isDark
                    ...
                    : 'bg-emerald-50 text-[#00A57A] border-emerald-200 hover:bg-[#00C896] hover:text-white'
                }`}
              >
